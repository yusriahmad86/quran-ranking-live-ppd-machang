...
export default function AnalisaKeseluruhanPage() {
  ...
  async function loadAnalysis() {
    try {
      ...
      const activeSchools: School[] = (schoolData || []).map((school: any) => ({
        id: school.id,
        name: school.name,
        code: school.code || "",
      }));
      ...
      const participantIds = participants.map((student) => student.id);
      ...
      const readings: ReadingRecord[] = readingData || [];
      ...
      const readingMap = new Map<string, ReadingRecord>();

      readings.forEach((reading) => {
        if (!readingMap.has(reading.participant_id)) {
          readingMap.set(reading.participant_id, reading);
        }
      });

      const studentResults: StudentAnalysis[] = participants.map(
        (student) => {
          const school = schoolMap.get(student.school_id);
          const latestReading = readingMap.get(student.id) || null;

          const studentReadings = readings.filter(
            (reading) => reading.participant_id === student.id
          );

          const pagesToday = studentReadings.reduce((total, reading) => {
            const from = Number(reading.page_from || 0);
            const to = Number(reading.page_to || 0);
          ...
          const level = getLevel(Number(student.current_page || 0));
          ...
          return {
            ...
            has_read_today: studentReadings.length > 0,
            level: level.name,
      ...
      console.error(err);
      setError(err?.message || "Gagal mendapatkan data analisa.");
    } finally {
      setLoading(false);
    }
  }
  ...
  const filteredStudents = useMemo(() => {
    return students.filter((student) => {
      const schoolMatch =
        selectedSchool === "ALL" || student.school_id === selectedSchool;
  ...
  }, [students, selectedSchool, selectedLevel]);
  ...
  const statistics = useMemo(() => {
    ...
    const pagesToday = students.reduce(
      (totalPages, student) => totalPages + student.pages_today,
      0
    );

    const totalCurrentPages = students.reduce(
      (totalPages, student) =>
        totalPages + Number(student.current_page || 0),
      0
    );

    const averagePages = filled > 0 ? pagesToday / filled : 0;

    const grandmaster = students.filter(
      (student) => student.level === "GRANDMASTER"
    ).length;

    const heroic = students.filter(
      (student) => student.level === "HEROIC"
    ).length;

    const diamond = students.filter(
      (student) => student.level === "DIAMOND"
    ).length;

    const platinum = students.filter(
      (student) => student.level === "PLATINUM"
    ).length;

    const gold = students.filter(
      (student) => student.level === "GOLD"
    ).length;

    const silver = students.filter(
      (student) => student.level === "SILVER"
    ).length;

    const bronze = students.filter(
      (student) => student.level === "BRONZE"
    ).length;
  ...
  const schoolAnalysis = useMemo<SchoolAnalysis[]>(() => {
    return schools
      .map((school) => {
        ...
        const filled = schoolStudents.filter(
          (student) => student.has_read_today
        ).length;

        const pages = schoolStudents.reduce(
          (total, student) => total + student.pages_today,
          0
        );
        ...
        return {
          ...
          filled,
          notFilled: schoolStudents.length - filled,
          pages,
        };
      })
      .filter((school) => school.total > 0)
      .sort((a, b) => {
        if (b.pages !== a.pages) {
        ...
        <main className="mx-auto max-w-7xl px-4 py-6">
          ...
          <div className="mb-6 hidden print:block">
            <div className="border-b-2 border-black pb-4 text-center">
              ...
              <p className="mt-2 text-sm font-bold">
                Tarikh: {formatDateMalay(selectedDate)}
              </p>
            </div>
          </div>
          ...
          <div className="mb-4 hidden print:block">
            <div className="flex justify-between border-b border-black pb-2 text-sm">
              ...
              <span>
                <strong>Level:</strong>{" "}
                {selectedLevel === "ALL" ? "Semua Level" : selectedLevel}
              </span>
            </div>
          </div>
          ...
          {error && (
            <div className="mb-6 rounded-2xl border border-red-500/40 bg-red-500/10 p-4 text-red-300">
              <div className="font-bold">❌ Ralat</div>
          ...
          {loading ? (
            <div className="rounded-2xl border border-slate-800 bg-slate-900 p-12 text-center">
              ...
              <p className="mt-3 font-bold">Sedang menyediakan analisa...</p>
            </div>
          ) : (
            <>
              {/* STATISTICS */}
              <section className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 print:grid-cols-4">
                ...
                <div className="rounded-2xl border border-blue-800 bg-blue-950/30 p-5 print:border-black print:bg-white">
                  ...
                  <div className="mt-2 text-4xl font-black text-blue-400 print:text-black">
                    {statistics.averagePages.toFixed(1)}
                  </div>
                ...
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-400 print:text-black">
                      {statistics.filled} daripada {statistics.total} murid
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 print:border-black print:bg-white">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[700px] text-sm">
                      ...
                      <tbody>
                        {schoolAnalysis.map((school, index) => {
                          const percentage =
                            school.total > 0
                              ? Math.round(
                                  (school.filled / school.total) * 100
                                )
                              : 0;
              ...
              <section>
                <div className="mb-4 flex items-end justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-slate-400 print:text-black">
                      Memaparkan {filteredStudents.length} murid
                    </p>
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900 print:border-black print:bg-white">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-[1000px] text-sm">
                      <thead>
                        <tr className="border-b border-slate-800 bg-slate-800 print:border-black print:bg-white">
                          <th className="px-3 py-3 text-center">Bil.</th>
                      ...
                      <tbody>
                          ...
                          >
                            ...
                            <td className="px-3 py-3 text-center">
                              <span className="inline-flex items-center gap-1 font-bold">
                                {student.level_icon} {student.level}
                              </span>
                            </td>
                            ...
                            <td className="px-4 py-3 text-center text-xs text-slate-400 print:text-black">
                              {student.latest_reading
                                ? formatDateTime(
                                    student.latest_reading.created_at
                                  )
                                : "-"}
                            </td>
                          </tr>
                        ...
                        {filteredStudents.length === 0 && (
                          <tr>
                            ...
                            >
                              Tiada murid ditemui berdasarkan tapisan yang
                              dipilih.
                            </td>
                          </tr>
                        )}
